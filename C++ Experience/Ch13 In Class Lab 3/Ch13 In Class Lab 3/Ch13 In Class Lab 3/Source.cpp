...
int main()
{
	...
	cout << " Retrienving Data from tmeperature file:" << endl;
	list.getList(tempData);

	// Fill in the code that calls the printList function.
	cout << "Will now print list" << endl;
	list.printList();
...
void FloatList::getList(ifstream& td)
{
	// The getList function reads the data values from a data file into the values array of the class FloatList
	while (td)
	{
		td >> values[length];
		cout << values[length] << endl;
	}
...
{
	// The printList function prints to the screen the data in the values array of the class FloatList
...
//Dominga Lapurkaite, Staje Kankaite, Diego De Los Santos